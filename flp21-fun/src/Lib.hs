...
import Data.Containers.ListUtils (nubOrd)  -- n*log(d), nub has n^2!
...
checkIfValidRule badRule = error ("Spatna syntaxe pravidla: " ++ badRule)
...
generateRules _ _ _ = error "Chyba v prevodu pravide z RLG do RRG."  
...
convertToNFA rrg = do
  ...
  return NFA { states = [0.. length (nonterminals rrg) - 1],
